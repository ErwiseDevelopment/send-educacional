...
<main class="relative overflow-hidden">
    ...
    <section class="sup-clara-2 relative pt-20 pb-16 overflow-hidden">
        ...
        <div class="container mx-auto px-6 max-w-4xl text-center relative z-10">
            <div class="flex justify-center items-center gap-2 text-xs txt mb-7 font-semibold uppercase tracking-widest">
                ...
                <span style="color:<?php echo esc_attr( $se_cor_t ); ?>">Educação Básica e Média</span>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-3">
                <button data-track="cta-hero-educacao-basica" onclick="abrirDemo('educacao-basica')" class="gbtn txt-forte font-bold px-8 py-4 rounded-2xl text-lg w-full sm:w-auto transition-all hover:-translate-y-0.5">Solicitar demonstração</button>
                <a href="#catalogo" class="w-full sm:w-auto px-8 py-4 rounded-2xl text-lg font-semibold txt-forte glass hover:bg-white/10 transition">Ver tudo o que o sistema faz</a>
            </div>
            <p class="txt-fraco text-sm mt-5">Demonstração gratuita · sem compromisso · com um especialista em educação básica</p>
        </div>
    </section>
    ...
    <section class="relative py-16">
        <div class="container mx-auto px-6 max-w-5xl">
            <div class="glass rounded-[2rem] p-8 md:p-10 reveal">
                <p class="text-xs font-bold uppercase tracking-widest txt mb-6 text-center">Se isso acontece na sua escola, é sistema, não é equipe</p>
                <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
                    <?php foreach ( array(
                        ...
                        printf(
                            '<div><div class="w-10 h-10 rounded-xl bg-rose-500/10 border border-rose-500/25 text-rose-600 flex items-center justify-center mb-4"><svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4m0 4h.01M10.3 3.9L1.8 18a2 2 0 001.7 3h17a2 2 0 001.7-3L13.7 3.9a2 2 0 00-3.4 0z"></path></svg></div><h3 class="font-bold txt-forte text-sm mb-2">%s</h3><p class="txt text-sm leading-relaxed">%s</p></div>',
    ...
    <section class="relative py-20">
        <div class="container mx-auto px-6 max-w-6xl grid lg:grid-cols-2 gap-12 items-center reveal">
            <div>
                <span class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full glass text-[11px] font-bold uppercase tracking-widest mb-5" style="color:<?php echo esc_attr( $se_cor_t ); ?>">Campanha de rematrícula</span>
                ...
                <button data-track="cta-rematricula" onclick="abrirDemo('educacao-basica')" class="gbtn txt-forte font-bold px-7 py-3.5 rounded-2xl transition-all hover:-translate-y-0.5">Ver a campanha na demonstração</button>
            </div>
            ...
            <div class="sup-escura sup-escura-2 rounded-2xl overflow-hidden border linha cardring">
                <div class="h-10 flex items-center px-4 gap-2 border-b linha bg-black/25">
                    ...
                    <span class="w-2.5 h-2.5 rounded-full bg-slate-600"></span>
                    <span class="w-2.5 h-2.5 rounded-full bg-slate-600"></span>
                    <span class="mx-auto text-[11px] txt font-semibold bg-bloco border linha rounded-full px-4 py-1">app.sendeducacional.com.br/rematricula</span>
                </div>
                <div class="p-5">
                    <p class="text-sm font-extrabold txt-forte mb-3">Rematrícula 2027 &middot; Escola Exemplo</p>
                    <div class="grid grid-cols-3 gap-3 mb-4">
                        <?php foreach ( array(
                            ...
                            printf( '<div class="rounded-xl bg-white/[.03] border linha p-3"><p class="text-[10px] txt font-semibold">%s</p><p class="text-2xl font-extrabold txt-forte mt-1">%s</p></div>', esc_html( $k[0] ), esc_html( $k[1] ) );
                        } ?>
                    </div>
                    <div class="rounded-xl bg-white/[.03] border linha p-4 mb-4">
                        <p class="text-[10px] txt font-semibold mb-3">Adesão por série</p>
                        <div class="space-y-2">
                            <?php foreach ( array(
                                array( '1º ao 5º ano', 89 ), array( '6º ao 9º ano', 76 ), array( '1ª série EM', 71 ), array( '2ª série EM', 68 ), array( '3ª série EM', 54 ),
                            ) as $p ) {
                                printf(
                                    '<div class="flex items-center gap-2 text-[10px]"><span class="w-24 txt truncate">%s</span><div class="flex-1 h-1.5 rounded bg-bloco"><div class="h-full rounded" style="width:%d%%;background:%s"></div></div><span class="w-8 text-right txt-fraco font-semibold">%d%%</span></div>',
                                    esc_html( $p[0] ), (int) $p[1], esc_attr( $se_cor ), (int) $p[1]
                                );
                            } ?>
                        </div>
                    </div>
                    <p class="text-[10px] txt-fraco text-center uppercase tracking-widest font-bold">Dados ilustrativos</p>
                </div>
            </div>
        </div>
    </section>
                ...
                <div class="grid md:grid-cols-3 gap-8 items-start">
                    <div class="md:col-span-1">
                        <div class="w-12 h-12 rounded-2xl flex items-center justify-center mb-5" style="background:<?php echo esc_attr( $se_cor ); ?>22;border:1px solid <?php echo esc_attr( $se_cor ); ?>55">
                            <svg class="w-6 h-6" style="color:<?php echo esc_attr( $se_cor_t ); ?>" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m5.6-4A12 12 0 0112 2.9 12 12 0 013.4 6 12 12 0 003 9c0 5.6 3.8 10.3 9 11.6 5.2-1.3 9-6 9-11.6 0-1-.1-2-.4-3z"></path></svg>
                        </div>
                        <h2 class="titulo text-2xl leading-tight mb-3">Regulação de escola é outra</h2>
                        <p class="txt leading-relaxed">Educação básica não responde ao MEC do mesmo jeito que uma faculdade: quem regula é o Conselho Estadual ou Municipal de Educação e a Secretaria de Educação. O sistema fala essa língua.</p>
                    </div>
                    <div class="md:col-span-2 grid sm:grid-cols-2 gap-4">
                        <?php foreach ( array(
                            ...
                            printf(
                                '<div class="rounded-2xl bg-white/[.03] border linha p-5"><h4 class="font-bold txt-forte text-sm mb-2">%s</h4><p class="txt text-sm leading-relaxed">%s</p></div>',
    ...
    <section class="relative py-20">
        <div class="container mx-auto px-6 max-w-5xl">
            <div class="glass rounded-[2.5rem] p-10 md:p-14 text-center cardring reveal">
                ...
                <p class="text-lg txt max-w-2xl mx-auto mb-8">Sem apresentação genérica: a gente pega a sua rematrícula, o seu boletim e a sua cobrança e mostra como ficam dentro do sistema.</p>
                <button data-track="cta-final-educacao-basica" onclick="abrirDemo('educacao-basica')" class="gbtn txt-forte font-bold px-9 py-4 rounded-2xl text-lg transition-all hover:-translate-y-0.5">Solicitar demonstração</button>
                <p class="txt-fraco text-sm mt-4">Gratuita · sem compromisso · com um especialista em educação básica</p>
            </div>
